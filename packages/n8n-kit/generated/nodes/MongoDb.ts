// GENERATED FILE, DO NOT EDIT
// Generated from 'vendor/n8n/packages/nodes-base/nodes/MongoDb/MongoDb.node.ts' node

export const name = "mongoDb" as const;
export const description =
	"Find, insert and update documents in MongoDB" as const;
export const version = 1.2 as const;
export const defaults = { name: "MongoDB" } as const;
export const credentials = [
	{ name: "mongoDb", required: true, testedBy: "mongoDbCredentialTest" },
] as const;

/**
 * Find, insert and update documents in MongoDB
 */
export type MongoDbNodeParameters = {};
